...
export function LyricsQueuePanel({
  ...
  return (
    <div className={cn('flex flex-col h-full bg-secondary/20 border-l overflow-hidden', className)}>
      ...
      {showLyrics && (
        <div 
          className={cn('min-h-0 overflow-hidden flex flex-col')}
          style={{ flex: showQueue ? 2 : 1 }}
        >
          {renderLyrics()}
        </div>
      )}
      ...
      {showQueue && (
        <div 
          className="border-t flex flex-col min-h-0 bg-background/40"
          style={{ flex: showLyrics ? 1.5 : 1 }}
        >
